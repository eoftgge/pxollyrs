...
impl PxollyTools {
	pub async fn new(config: PxollyConfig) -> PxollyResult<Self> {
		...
		tools.set_ip().await?;
		tools.set_confirmation_code().await?;
		Ok(tools)
	}
	...
	async fn set_confirmation_code(&mut self) -> PxollyResult<()> {
		...
		self.confirmation_code = Some(response
			.get("response").ok_or_else(|| PxollyError::None)?
			.get("confirmation_code").ok_or_else(|| PxollyError::None)?
			.as_str().ok_or_else(|| PxollyError::None)?
			.to_string());
		Ok(())
	}
	...
	async fn set_ip(&mut self) -> PxollyResult<()> {
		...
		self.ip = Some(format!(
			"{}:{}",
			response["origin"].as_str().ok_or_else(|| PxollyError::None)?,
			self.config.port
		));
		Ok(())
	}
	...
	pub async fn make_webhook(&self) -> PxollyResult<()> {
		...
		tokio::time::sleep(Duration::from_secs(1)).await; // немного костыльно, но сойдет
