use crate::utils::{database::DatabaseJSON, PxollyTools, vec_to_string};
...
impl PxollyHandler {
	async fn add_chat(&self, event: PxollyEvent) -> ResultHandle {
		...
		let chat_id = event.object.chat_id
			.ok_or_else(|| PxollyError::None)?;
		let chat_uid = event.object.chat_uid
			.ok_or_else(|| PxollyError::None)?;
	...
	async fn invite_user(&self, event: PxollyEvent, chat_id: u64) -> ResultHandle {
		...
		self.api_client.api_request("messages.addChatUser", params).await?;
		Ok(PxollyResponse::Success)
	}
	...
	pub async fn handle(&self, event: PxollyEvent) -> PxollyResult<PxollyResponse> {
		let chat_id = match event.object.chat_id.as_ref() {
			...
			None if event.c_type == "confirmation" =>
				return Ok(PxollyResponse::ConfirmationCode(self.tools.get_confirmation_code())),
			None =>
				return Ok(PxollyResponse::UnknownUIDOrNoSupport),
			Some(_) if event.c_type == "add_chat" =>
				return Ok(self.add_chat(event).await?),
